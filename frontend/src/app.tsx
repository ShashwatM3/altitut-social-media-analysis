...
const DASHBOARD_PAGES: Array<{ id: DashboardPage; label: string; title: string; description: string }> = [
  {
    id: 'competitor-scout',
    label: 'Tool 1',
...
function getReviewDecision(item: { approved?: boolean; rejected?: boolean }, localDecision?: ReviewDecision): ReviewStatus {
...
function readStoredAltitutContext(): string {
  ...
  try {
    ...
    return raw && raw.trim().length ? raw : DEFAULT_ALTITUT_CONTEXT;
  } catch {
    return DEFAULT_ALTITUT_CONTEXT;
  }
}
...
function readStoredActivePage(): DashboardPage {
  ...
  try {
    ...
    if (raw === 'competitor-scout' || raw === 'posts-analysis' || raw === 'settings') {
      return raw;
    }
  } catch {
    return 'competitor-scout';
...
function renderCompetitorSubtitle(item: { id: string; website?: string | null }): ReactNode {
  return (
...
function App() {
  const [activePage, setActivePage] = useState<DashboardPage>(() => readStoredActivePage());
  const [altitutContext, setAltitutContext] = useState<string>(() => readStoredAltitutContext());
  ...
  const [lastScoutCandidateCount, setLastScoutCandidateCount] = useState<number | null>(null);
  const [reviewState, setReviewState] = useState<ReviewState>(() => readStoredReviewState());

  useEffect(() => {
    try {
      if (Object.keys(reviewState.competitors).length === 0 && Object.keys(reviewState.posts).length === 0) {
        window.localStorage.removeItem(REVIEW_STORAGE_KEY);
        return;
      }
  ...
  const activePageMeta = DASHBOARD_PAGES.find((page) => page.id === activePage) ?? DASHBOARD_PAGES[0];
  ...
  async function runCompetitorScout(event: FormEvent<HTMLFormElement>) {
    ...
    setNotice('');
    try {
      const response = await fetchJson<ApiEnvelope<Competitor>>('/competitor-scout', {
        method: 'POST',
        body: JSON.stringify({
      ...
      setLastScoutCandidateCount(response.candidate_count ?? response.candidates?.length ?? null);
  ...
  async function rejectCompetitor(id: string) {
    ...
    try {
      ...
      setSelectedCompetitorIds((current) => current.filter((competitorId) => competitorId !== id));
  ...
  async function refactorDatabase() {
    ...
    setNotice('');
    try {
      const response = await fetchJson<DatabaseRefactorResponse>('/database/refactor', {
        method: 'POST',
      });
      ...
      setNotice(summary ? `Database records deleted: ${summary}.` : 'Database records deleted.');
      await loadDashboard({ preserveNotice: true });
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Unable to refactor database.');
    } finally {
      setBusy(null);
    }
  }
    ...
    <div className="app-shell">
      <aside className="side-nav">
        <div className="side-nav-brand">
          ...
          <p className="lead">IBM / Carbon-inspired dashboard with a focused product workflow.</p>
        </div>
        ...
        <div className="side-nav-meta">
          ...
          <p className="muted">
            The scout uses the saved Altitut context from Settings. If no override is provided, the backend falls
      ...
      <div className="app-content">
        <header className="page-header">
          ...
          <div className="page-header-meta" aria-label="Live dashboard summary">
            <span className="pill neutral">Live data</span>
            <span className="pill success">{approvedCompetitors.length} approved competitors</span>
            <span className="pill neutral">{analysisResult.post_count ?? analysisResult.posts?.length ?? 0} analyzed posts</span>
          </div>
        </header>
            ...
            <section className="panel" id="competitor-scout">
              <div className="panel-heading">
                ...
                <div className="inline-actions">
                  ...
                  >
                    {busy === 'database-refactor' ? 'Deleting...' : 'Refactor Database'}
                  </button>
                </div>
              </div>
              ...
              <form className="stack" onSubmit={runCompetitorScout}>
                <p className="muted">
                  The scout uses the Altitut context saved in Settings and sends it to the LLM pipeline.
                </p>
                <div className="inline-hint">
                  <span className="pill neutral">Context active</span>
                  <span className="muted">{currentContext.slice(0, 140)}{currentContext.length > 140 ? '…' : ''}</span>
                </div>
                ...
                {lastScoutCandidateCount !== null ? (
                  <div className="inline-hint scout-result-count" aria-live="polite">
                    <span className="pill neutral">Latest scout result</span>
                    <span className="muted">
                      {lastScoutCandidateCount} competitor{lastScoutCandidateCount === 1 ? '' : 's'} returned.
                    </span>
                  </div>
                ) : null}
              </form>
                ...
                {dashboardLoading ? (
                  ...
                  competitorRows.map(({ competitor, status, localDecision }) => (
                    <ActionCard
                      ...
                      headerRight={<span className="muted">{competitor.social_links ? Object.keys(competitor.social_links).length : 0} social link(s)</span>}
                      footer={
                        <ReviewControls
                          status={status}
            ...
            <section className="panel" id="posts-analysis">
              ...
              <form className="stack" onSubmit={runPostsAnalysis}>
                <div className="grid two-up">
                  ...
                  <div className="stack">
                    <span className="field-label">Approved competitors to analyze</span>
                    <div className="selector-grid">
                      ...
                      {!approvedCompetitors.length ? <p className="muted">Approve competitors first to enable posts analysis.</p> : null}
                    </div>
                    <div className="inline-hint">
                      <span className="muted">
          ...
          <main className="dashboard-stack">
            <section className="panel settings-panel" id="settings">
              <div className="panel-heading">
                ...
                <button type="button" className="secondary" onClick={resetAltitutContext}>
                  Reset to default
                </button>
              </div>
              ...
              <div className="settings-meta">
                <p className="muted">
                  This value is preloaded from the backend default and saved locally in this browser. The scout sends
                  it with every run.
                </p>
                <p className="muted">Current length: {currentContext.length} characters.</p>
              </div>
            </section>
          </main>
        ) : null}
