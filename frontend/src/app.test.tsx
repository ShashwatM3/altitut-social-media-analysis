...
function installDashboardFetch(options: {
  ...
  const { competitors, posts, analysisPosts = [], scoutResultCount = competitors.length } = options;
  ...
  const fetchMock = async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.endsWith('/integrations/llm/status')) {
      return jsonResponse({
        ...
        details: { config: { model: 'gpt-4.1-mini', base_url: 'https://api.openai.com/v1' }, mode: 'remote' },
      });
    }
    ...
    if (url.endsWith('/reject') && method === 'POST') {
      ...
      if (match) {
        ...
        const index = currentCompetitors.findIndex((competitor) => competitor.id === competitorId);
        if (index !== -1) {
...
function waitForDashboard() {
  return waitFor(() => {
    expect(screen.getByRole('navigation', { name: 'Primary pages' })).toBeTruthy();
    expect(screen.getByRole('button', { name: 'Run competitor scout' })).toBeTruthy();
  });
}
...
describe('dashboard states', () => {
  it('renders the side-nav with three pages and the competitor scout default view', async () => {
    ...
    expect(screen.getByText('Enterprise social intelligence dashboard')).toBeTruthy();
    expect(screen.getAllByRole('heading', { name: 'Competitor Scout' }).length).toBeGreaterThan(0);
  });
  ...
  it('shows a destructive Refactor Database button that clears all records after confirmation', async () => {
    ...
    expect(window.localStorage.getItem('altitut.dashboard.review-state.v1')).toBeNull();
  ...
  it('removes a dismissed competitor completely from the dashboard and local memory', async () => {
    ...
    expect(window.localStorage.getItem('altitut.dashboard.review-state.v1')).toBeNull();
    const rejectRequest = dashboard.requests.find((request) => request.url.endsWith('/reject') && request.method === 'POST');
  ...
  it('shows the competitor count directly under the scout button after the run completes', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Run competitor scout' }));
  ...
  it('clears saved review decisions when the database is refactored', async () => {
    ...
    window.localStorage.setItem(
      'altitut.dashboard.review-state.v1',
      JSON.stringify({ competitors: { 'memory-target': 'approved' }, posts: {} }),
    );
  ...
  it('lets settings override the Altitut context used by competitor scout', async () => {
    ...
    await waitFor(() => {
      const scoutRequest = dashboard.requests.find(
        (request) => request.url.endsWith('/competitor-scout') && request.method === 'POST',
      );
  ...
  it('runs post analysis from approved competitors and shows returned post cards', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('button', { name: 'Analyze posts' })).toBeTruthy();
    });
